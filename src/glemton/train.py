...
def train(config_path: str):
    ...
    model = model_from_config(cfg).to(device=device)
    ...
    while tokens_seen < total_tokens:
        ...
        if tokens_seen >= next_ckpt:
            ...
            torch.save(
                {"model": model.state_dict(), "step": step, "tokens": tokens_seen, "cfg": cfg},
                path,
            )
    ...
    torch.save({"model": model.state_dict(), "step": step, "tokens": tokens_seen, "cfg": cfg}, path)
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        print("usage: python -m glemton.train <config.yaml>")
        sys.exit(2)
    train(sys.argv[1])
